import { Link, useNavigate, useParams } from "react-router-dom";
import { useState, useEffect } from "react";
import * as React from "react";
import DashboardLayout from "examples/LayoutContainers/DashboardLayout";
import DashboardNavbar from "examples/Navbars/DashboardNavbar";

// @mui material components
import Card from "@mui/material/Card";
import Grid from "@mui/material/Grid";
import Icon from "@mui/material/Icon";
import InputLabel from "@mui/material/InputLabel";
import CircularProgress from "@mui/material/CircularProgress";
import TextField from "@mui/material/TextField";

// Material Dashboard 2 React components
import MDButton from "components/MDButton";
import MDBox from "components/MDBox";
import MDTypography from "components/MDTypography";
import MDInput from "components/MDInput";
import { axios_get, axios_post } from '../../../axios';
import { ToastMassage } from '../../../toast';
import { FormControl, FormControlLabel, MenuItem, Radio, RadioGroup, Select } from "@mui/material";

function EditNew() {
  const params = useParams();

  const navigate = useNavigate();
  const [formError, setFormError] = useState({});
  const [isSubmit, setisSubmit] = useState(false);
  const [countries, setCountries] = useState([]);
  let user_data = JSON.parse(localStorage.getItem("user_data"));

  const [formData, setFormData] = useState({
    lastname: "",
    firstname: "",
    cusemail1: "",
    subemail2: "",
    cusemail3: "",
    cusemail4: "",
    country_id: "",
    mobile: "",
    status: "1",
    customer_code: "",
    customer_address_1: "",
    customer_address_2: "",
    cussname: "",
    cuscomname: "",
    custax1: "",
    custax2: "",
    custax3: "",
    cusauth: "",
    cusfax: "",
    cusdob: "",
    cusanndt: "",
    custoll: "",
    cusconpername: "",
    cusconpername2: "",
    cusconpername3: "",
    cusphone: "",
    cusphone2: "",
    cusphone3: "",
    mobile2: "",
    custaxdt1: "",
    custaxdt2: "",
    note1: "",
    note2: "",
    addedby: `${user_data.firstname} ${user_data.lastname}`,
    createddt: new Date().toLocaleString(),
    category: "",
    custitle: "",
    cusadd3: ""
  });
  const OrderNuberRange = async () => {
    let params = {
      function_for: "customer"
    }
    const response = await axios_post(true, "code_setting/get-next-comming-code", params);
    if (response) {
      if (response.status) {
        setFormData((prevData) => ({
          ...prevData,
          "customer_code": response.data.number_is,
        }));
      } else {
        ToastMassage(response.message, 'error')
      }
    }
  }
  const fetchcountryList = async () => {
    const response = await axios_get(true, "country/list-dropdown");
    if (response) {
      if (response.status) {
        setCountries(response.data);
      } else {
        ToastMassage(response.message, 'error');
      }
    }
  };
  const fetchOrderDetails = async () => {
    try {
      const response = await axios_post(true, 'customer/details', {
        id: params.id
      });
      if (response.status) {
        const orderData = response.data.users;
        const orderData1 = response.data;
        setFormData({
          ...formData,
          customer_code: orderData1.customer_code,
          name: orderData.name,
          lastname: orderData.lastname,
          firstname: orderData.firstname,
          mobile: orderData.mobile,
          email: orderData.email,
          country_id: orderData.country_id,
          customer_address_1: orderData1.customer_address_1,
          customer_address_2: orderData1.customer_address_2,
          status: orderData1.status === 1 ? "1" : "0",
        });

      } else {
        ToastMassage(response.message, 'error');
      }
    } catch (error) {
      console.error("Failed to fetch order details:", error);
    }
  };
  useEffect(() => {
    OrderNuberRange();
    fetchOrderDetails();
    fetchcountryList();
  }, []);

  const validation = (formData) => {
    let errors = {};

    if (!formData.firstname) errors.firstname = "FirstName is required";
    if (!formData.lastname) errors.lastname = "LastName is required";
    if (!formData.cussname) errors.cussname = "Customer Short Name is required";
    if (!formData.custax1) errors.custax1 = "Tax No 1 is required";
    if (!formData.cusemail1) {
      errors.cusemail1 = "Email is required";
    } else if (!/\S+@\S+\.\S+/.test(formData.cusemail1)) {
      errors.cusemail1 = "Email is invalid";
    }


    if (!formData.country_id) errors.country_id = "country is required";
    if (!formData.mobile) errors.mobile = "Mobile No is required";
    if (!/\S+@\S+\.\S+/.test(formData.subemail2)) {
      errors.subemail2 = "Email is invalid";
    }
    if (!formData.status) errors.status = "Status is required";

    if (!/\S+@\S+\.\S+/.test(formData.cusemail3)) {
      errors.cusemail3 = "Email is invalid";
    }
    if (!formData.customer_address_1) errors.customer_address_1 = "Address is required";
    if (!formData.createddt) errors.createddt = "Created Date is required";
    if (!formData.addedby) errors.addedby = "Added By is required";
    if (!formData.category) errors.category = "Category is required";
    if (!formData.custitle) errors.custitle = "Title is required";
    return errors;
  };

  const handleSubmit = async (event) => {
    setisSubmit(true);
    event.preventDefault();
    let errors = validation(formData);

    if (Object.keys(errors).length > 0) {
      setisSubmit(false);
      setFormError(errors);
    } else {
      setFormError({});
      const response = await axios_post(true, "customer/update", {
        ...formData,
        id: params.id
      });
      if (response) {
        setisSubmit(false);
        if (response.status) {
          ToastMassage(response.message, 'success');
          navigate("/customer");
        } else {
          ToastMassage(response.message, 'error');
        }
      }
    }
  };

  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData((prevData) => ({
      ...prevData,
      [name]: value,
    }));
  };

  const handleBack = () => {
    navigate("/customer");
  }

  return (
    <DashboardLayout>
      <DashboardNavbar />
      <MDBox pt={6} pb={3}>
        <Grid container spacing={6} justifyContent="center">
          <Grid item xs={12}>
            <form onSubmit={handleSubmit} method="POST" action="#">
              <Card>
                <MDBox
                  mx={2}
                  mt={-3}
                  py={3}
                  px={2}
                  variant="gradient"
                  bgColor="info"
                  borderRadius="lg"
                  coloredShadow="info"
                >
                  <Grid container spacing={0}>
                    <Grid item xs={2} mr={35}>
                      <MDTypography variant="h6" color="white">
                        <Icon fontSize="small">shopping_cart</Icon>
                        Add Customer
                      </MDTypography>
                    </Grid>

                    <Grid item xs={2} ml={40}>
                      <MDTypography component={Link} to="/customer">
                        <MDButton variant="gradient" color="light">
                          Back
                        </MDButton>
                      </MDTypography>
                    </Grid>
                  </Grid>
                </MDBox>
                <MDBox pt={4} pb={3} px={3}>
                  <MDBox>
                    <Grid container rowSpacing={2} columnSpacing={{ xs: 1, sm: 2, md: 1 }}>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Category</InputLabel>
                          <Select
                            variant="outlined"
                            name="cuscat"
                            value={formData.cuscat}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                          >
                            <MenuItem value="Walking">Walking</MenuItem>
                            <MenuItem value="Reference">Reference</MenuItem>
                            <MenuItem value="Corporate">Corporate</MenuItem>
                            <MenuItem value="Retail">Retail</MenuItem>
                          </Select>
                          {formError.cuscat && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cuscat}</MDTypography>}

                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Customer code</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="customer_code"
                            value={formData.customer_code}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            disabled
                            className="small-input"
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Title</InputLabel>
                          <Select
                            variant="outlined"
                            name="custitle"
                            value={formData.custitle}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                          >
                            <MenuItem value="Mr.">Mr.</MenuItem>
                            <MenuItem value="Miss">Miss</MenuItem>
                            <MenuItem value="Mrs">Mrs</MenuItem>
                            <MenuItem value="Dr">Dr</MenuItem>
                          </Select>
                          {formError.custitle && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.custitle}</MDTypography>}

                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Customer  Name</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusname"
                            value={formData.cusname}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 60 }}
                          />
                          {formError.cusname && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusname}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Customer Short Name</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cussname"
                            value={formData.cussname}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 60 }}
                          />
                          {formError.cussname && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cussname}</MDTypography>}
                        </MDBox>
                      </Grid>

                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Brand Name</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusbrand"
                            value={formData.cusbrand}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 120 }}
                          />
                          {formError.cusbrand && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusbrand}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Company Name</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cuscomname"
                            value={formData.cuscomname}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 120 }}
                          />
                          {formError.cuscomname && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cuscomname}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>country</InputLabel>
                          <Select
                            name="country_id"
                            value={formData.country_id}
                            onChange={handleChange}
                            className="small-input"
                          >
                            {countries?.map((country) => (
                              <MenuItem key={country.id} value={country?.id}>
                                {country?.name}
                              </MenuItem>
                            ))}
                          </Select>
                          {formError.country_id && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.country_id}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Tax No 1</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="custax1"
                            value={formData.custax1}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 60 }}
                          />
                          {formError.custax1 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.custax1}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Tax No 2</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="custax2"
                            value={formData.custax2}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 60 }}
                          />
                          {formError.custax2 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.custax2}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Tax No 3</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="custax3"
                            value={formData.custax3}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 60 }}
                          />
                          {formError.custax3 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.custax3}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Tax Authority</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusauth"
                            value={formData.cusauth}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 60 }}
                          />
                          {formError.cusauth && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusauth}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Fax No</InputLabel>
                          <MDInput
                            type="number"
                            variant="outlined"
                            name="cusfax"
                            value={formData.cusfax}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 20 }}
                          />
                          {formError.cusfax && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusfax}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Date Of Birth</InputLabel>
                          <MDInput
                            type="date"
                            name="cusdob"
                            variant="outlined"
                            className="small-input"
                            value={formData.cusdob}
                            onChange={handleChange}
                          />
                          {formError.cusdob && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusdob}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Anniversary Date</InputLabel>
                          <MDInput
                            type="date"
                            variant="outlined"
                            name="cusanndt"
                            value={formData.cusanndt}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                          />
                          {formError.cusanndt && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusanndt}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Toll Free</InputLabel>
                          <MDInput
                            type="number"
                            variant="outlined"
                            name="custoll"
                            value={formData.custoll}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 20 }}
                          />
                          {formError.custoll && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.custoll}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Contact Person name</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusconpername"
                            value={formData.cusconpername}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 60 }}
                          />
                          {formError.cusconpername && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusconpername}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Phone No</InputLabel>
                          <MDInput
                            type="number"
                            variant="outlined"
                            name="cusphone"
                            value={formData.cusphone}
                            onChange={(e) => {
                              const value = e.target.value;
                              if (/^\d{0,10}$/.test(value)) {
                                handleChange(e);
                              }
                            }}
                            inputProps={{ maxLength: 10 }}
                            className="small-input"
                          />
                          {formError.cusphone && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusphone}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Email Id </InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusemail1"
                            value={formData.cusemail1}
                            onChange={handleChange}
                            className="small-input"
                            inputProps={{ maxLength: 30 }}
                          />
                          {formError.cusemail1 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusemail1}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Mobile No</InputLabel>
                          <MDInput
                            type="number"
                            variant="outlined"
                            name="mobile"
                            value={formData.mobile}
                            onChange={(e) => {
                              const value = e.target.value;
                              if (/^\d{0,10}$/.test(value)) {
                                handleChange(e);
                              }
                            }}
                            inputProps={{ maxLength: 10 }}
                            className="small-input"
                          />
                          {formError.mobile && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.mobile}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Address 1</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="customer_address_1"
                            value={formData.customer_address_1}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 200 }}
                          />
                          {formError.customer_address_1 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.customer_address_1}</MDTypography>}
                        </MDBox>
                      </Grid>

                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Contact Person name2</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusconpername2"
                            value={formData.cusconpername2}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            className="small-input"
                            inputProps={{ maxLength: 60 }}
                          />
                          {formError.cusconpername2 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusconpername2}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Phone No 2</InputLabel>
                          <MDInput
                            type="number"
                            variant="outlined"
                            name="cusphone2"
                            value={formData.cusphone2}
                            onChange={(e) => {
                              const value = e.target.value;
                              if (/^\d{0,10}$/.test(value)) {
                                handleChange(e);
                              }
                            }}
                            inputProps={{ maxLength: 10 }}
                            className="small-input"
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Email Id 2</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="subemail2"
                            value={formData.subemail2}
                            onChange={handleChange}
                            className="small-input"
                            inputProps={{ maxLength: 30 }}
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Mobile No 2</InputLabel>
                          <MDInput
                            type="number"
                            variant="outlined"
                            name="mobile2"
                            value={formData.mobile2}
                            onChange={(e) => {
                              const value = e.target.value;
                              if (/^\d{0,10}$/.test(value)) {
                                handleChange(e);
                              }
                            }}
                            inputProps={{ maxLength: 10 }}
                            className="small-input"
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Address 2</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="customer_address_2"
                            value={formData.customer_address_2}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 200 }}
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Contact Person name3</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusconpername3"
                            value={formData.cusconpername3}
                            onChange={handleChange}
                            // sx={{ width: 300 }}
                            inputProps={{ maxLength: 60 }}
                            className="small-input"
                          />
                          {formError.cusconpername3 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusconpername3}</MDTypography>}
                        </MDBox>
                      </Grid>


                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Phone No 3</InputLabel>
                          <MDInput
                            type="number"
                            variant="outlined"
                            name="cusphone3"
                            value={formData.cusphone3}
                            onChange={(e) => {
                              const value = e.target.value;
                              if (/^\d{0,10}$/.test(value)) {
                                handleChange(e);
                              }
                            }}
                            inputProps={{ maxLength: 10 }}
                            className="small-input"
                          />
                        </MDBox>
                      </Grid>


                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Email ID 3</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusemail3"
                            value={formData.cusemail3}
                            onChange={handleChange}
                            className="small-input"
                            inputProps={{ maxLength: 30 }}
                          />
                          {formError.cusemail3 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusemail3}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Address 3</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusadd3"
                            value={formData.cusadd3}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 200 }}
                          />
                          {formError.cusadd3 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusadd3}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Other No</InputLabel>
                          <MDInput
                            type="number"
                            variant="outlined"
                            name="otherno"
                            value={formData.otherno}
                            onChange={(e) => {
                              const value = e.target.value;
                              if (/^\d{0,10}$/.test(value)) {
                                handleChange(e);
                              }
                            }}
                            inputProps={{ maxLength: 10 }}
                            className="small-input"
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Email ID 4</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="cusemail4"
                            value={formData.cusemail4}
                            onChange={handleChange}
                            className="small-input"
                            maxLength={30}
                          />
                          {formError.cusemail4 && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.cusemail4}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Note 1</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="note1"
                            value={formData.note1}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 50 }}
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Note 2</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="note2"
                            value={formData.note2}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 50 }}
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Note 3</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="note3"
                            value={formData.note3}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 50 }}
                          />
                        </MDBox>
                      </Grid>


                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Note 1</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="note1"
                            value={formData.note1}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 50 }}
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Note 2</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="note2"
                            value={formData.note2}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 50 }}
                          />
                        </MDBox>
                      </Grid>

                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Note 3</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="note3"
                            value={formData.note3}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 50 }}
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Date 1</InputLabel>
                          <MDInput
                            type="date"
                            name="custaxdt1"
                            variant="outlined"
                            className="small-input"
                            value={formData.custaxdt1}
                            onChange={handleChange}
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Date 2</InputLabel>
                          <MDInput
                            type="date"
                            name="custaxdt2"
                            variant="outlined"
                            className="small-input"
                            value={formData.custaxdt2}
                            onChange={handleChange}
                          />
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Added By</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="addedby"
                            value={formData.addedby}
                            onChange={(e) => handleChange(e)}
                            className="small-input"
                            inputProps={{ maxLength: 40 }}
                            disabled
                          />
                          {formError.addedby && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.addedby}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Created Date</InputLabel>
                          <MDInput
                            type="text"
                            variant="outlined"
                            name="createddt"
                            value={formData.createddt}
                            className="small-input"
                            disabled
                          />
                          {formError.createddt && <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>{formError.createddt}</MDTypography>}
                        </MDBox>
                      </Grid>
                      <Grid item xs={12} sm={4}>
                        <MDBox pb={2}>
                          <InputLabel sx={{ mb: 1 }}>Status</InputLabel>
                          <Select
                            name="status"
                            value={formData.status}
                            onChange={handleChange}
                            sx={{ width: 250, height: 45 }}
                          >
                            <MenuItem value="1">Active</MenuItem>
                            <MenuItem value="0">Inactive</MenuItem>
                          </Select>
                          {formError.status && (
                            <MDTypography color="error" sx={{ fontSize: '14px', mt: "10px" }}>
                              {formError.status}
                            </MDTypography>
                          )}
                        </MDBox>
                      </Grid>
                      <Grid container spacing={2} justifyContent="right" sx={{ mt: 1, mb: 2 }}>
                        <Grid item xs={2} ml={3}>
                          <MDBox sx={{ display: 'flex' }}>
                            <MDButton variant="gradient" disabled={isSubmit} color="info" type="submit" fullWidth>
                              {isSubmit ?
                                <CircularProgress color="white" size={24}
                                  sx={{
                                    position: 'absolute',
                                    top: '50%',
                                    left: '50%',
                                    marginTop: '-12px',
                                    marginLeft: '-12px',
                                  }} />
                                : 'Save'
                              }
                            </MDButton>
                            <MDButton variant="gradient" disabled={isSubmit} color="secondary" fullWidth sx={{ marginLeft: '15px' }} onClick={handleBack}>
                              Cancel
                            </MDButton>
                          </MDBox>
                        </Grid>
                      </Grid>
                    </Grid>
                  </MDBox>
                </MDBox>
              </Card>
            </form>
          </Grid>
        </Grid>
      </MDBox>
    </DashboardLayout>
  );
}

export default EditNew;
