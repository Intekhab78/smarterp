...
export default function Van_To_Van_Transfer() {
  const [data, setData] = useState([]);
  const [open, setOpen] = useState(false);
  const [actionopen, setActionpen] = useState(false);
  const [inactiveopen, setInactiveopen] = useState(false);
  const [SelectedUUID, setSelectedUUID] = useState([]);
  const [vantovanData, setvantovanData] = useState({});
  ...
  const handleClickOpen = (vantovanData) => {
    setvantovanData(vantovanData);
    setOpen(true);
  };
  const handleClose = () => {
    setOpen(false);
  ...
  const [isDeleting, setIsDeleting] = useState(false);
   ...
   const handleDelete = async () => {
    ...
    const { uuid } = vantovanData;
    setOpen(false);
    await axios
      .get("van-to-van-transfer/delete/" + uuid)
      .then((response) => {
        getdetails();
    ...
    },
    {
      ...
      renderCell: (params) => (
        <MDButton
          ...
          color="info"
          component={Link}
          to={`/van-to-van-transfer/edit/${params.row.uuid}`}
        >
      ...
      >
        ...
        <DialogContent className="dialog-content">
          <Typography gutterBottom style={{ fontSize: "20" }}>
            Are you sure want to delete this record {vantovanData.name}??
          </Typography>
        </DialogContent>
              ...
              >
                <Grid container spacing={1}>
                  ...
                  <Grid item xs={1} ml={25}>
                    <MDTypography component={Link} to="/master/van-to-van-transfer/add">
